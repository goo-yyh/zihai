    ...
    <div className="space-y-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Created {formatDate(project.createdAt)} · Public slug /p/
            {project.slug}
          </p>
        </div>
      ...
      {project.status === "pending" ? (
        <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-medium text-amber-900">
          This project is in review. You can still edit it, but changes remain
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add between 1 and 3 images. The first image becomes the listing
            cover.
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-4">
          ...
          {project.status === "approved" ? (
            <Button asChild size="sm">
              <Link href={`/dashboard/projects/${project.id}/iterations/new`}>
                <Plus className="size-4" /> New iteration
              </Link>
            </Button>
          ) : null}
        </CardHeader>
