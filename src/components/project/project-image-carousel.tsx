...
import { ProjectImageLightbox } from "@/components/project/project-image-lightbox";
...
export function ProjectImageCarousel({
  ...
  const closeExpandedImage = useCallback(() => setExpanded(false), []);
        ...
        >
          {currentImage ? (
            <div className="relative h-full w-full">
              <button
                type="button"
                disabled={!currentImageLoaded || currentImageFailed}
                onClick={() => setExpanded(true)}
                aria-label={t("Enlarge image")}
                aria-haspopup="dialog"
                aria-expanded={expanded}
                title={t("Enlarge image")}
                className="group absolute inset-0 cursor-zoom-in focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring disabled:cursor-default"
              >
                <ProductScreenshot
                  key={currentImage.id}
                ...
                />
                {currentImageLoaded && !currentImageFailed ? (
                  <span className="pointer-events-none absolute bottom-3 right-3 z-10 rounded-full bg-foreground/75 p-2 text-white shadow-sm backdrop-blur transition-transform group-hover:scale-105">
