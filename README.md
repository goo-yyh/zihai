...
- 每个项目支持 1～5 张有序截图
- 网站和 GitHub 之外，项目也可上传一张小程序二维码作为访问目标；首页、详情页和管理员审核页均可查看
...
- 创建草稿时只需名称和描述，可在编辑页继续补充网站地址、GitHub 仓库地址或二维码。只有草稿可以暂时没有目标；提交后所有状态都必须至少保留这三类目标之一，也可以任意组合。
- 项目在提交及审核时必须拥有 1～5 张 JPEG、PNG 或 WebP 截图。
...
- 系统必须始终保留至少一名管理员。
- 除 [项目建议与通知设计](specs/notification.md) 明确无需审核的公开项目建议外，项目名称、描述、链接、截图和二维码等公开字段都必须先经过审核。

上述规则不仅存在于界面层：鉴权、所有权和状态转换会在服务端重新检查；按状态生效的项目目标要求、二维码 URL/Pathname 配对、唯一点赞、所有者关系和并发截图上限由 PostgreSQL 最终兜底。二维码是项目目标，不计入仍然必需的 1～5 张截图。

二维码目标的状态矩阵、上传与替换顺序、公开交互和验收范围见 [项目二维码目标设计方案](specs/project-qr-code.md)。
...
执行迁移前必须人工检查生成的 SQL。不要编辑已经进入共享环境或生产环境的迁移，也不要在应用启动时自动修改生产数据库。迁移除表结构外，还包含按状态生效的项目目标约束、二维码 URL/Pathname 配对、图片策略、所有者校验和并发截图数量触发器。
...
浏览器不会直接获得 Blob 密钥。上传流程会签发与用户、资源、路径、MIME 类型和过期时间绑定的上传意图，Provider Token 使用同一过期时间；二维码意图还绑定签发时的 pathname 与项目版本，避免删除后的延迟回调重新挂回旧二维码。Blob 写入成功后，浏览器必须等待认证完成接口再次验证签名意图和 Blob 返回的真实 pathname、MIME、大小，并把 Provider 返回的规范 URL 写入 PostgreSQL，之后界面才提示成功并刷新。Vercel 的完成回调复用同一套幂等服务作为兜底，本地开发不依赖公网回调地址。

- 头像：最多 1 张，单张不超过 2 MiB。
- 项目截图：1～5 张，单张不超过 5 MiB。
- 项目二维码：最多 1 张，单张不超过 5 MiB，不占用截图数量。
- 支持格式：JPEG、PNG、WebP。
- 新文件写入数据库失败时，会在同一 pathname 的完成互斥锁内确认数据库没有现存引用，再补偿删除刚上传的 Blob；引用检查失败时宁可保留孤儿对象，也不会误删已引用对象。
- 头像替换在数据库提交后再尽力清理旧 Blob，避免删除已经被引用的新文件。
- 二维码替换会先上传新 Blob，再锁定项目并原子更新 URL/Pathname 和审核生命周期；提交后才尽力清理旧 Blob。
- 删除项目或账号时，在同一数据库事务中锁定并删除关系数据，同时收集二维码、头像和项目截图的精确 Pathname；事务提交后再尽力清理这些 Blob，清理失败不会恢复已删除的关系数据。

PostgreSQL 与 Blob 无法共享事务，新增文件工作流时必须明确写出数据库提交与对象清理顺序。

## 测试与质量门禁

当前单元测试覆盖内容生命周期、提交截图数量与项目目标规则、输入规范化、安全返回地址、Slug、图片上传策略、公开项目筛选参数、游标分页和预期错误映射。GitHub Actions 在 Pull Request 及推送到 `main` 时执行与本地一致的格式、Lint、类型、测试和构建检查。
