...
本文定义 zihAI 的“项目建议”和“站内通知”功能，包括产品规则、页面交互、数据模型、权限边界、事务与并发策略、缓存失效、实施步骤、测试范围和上线顺序。本文是后续实现与验收的依据，不包含本次文档任务之外的功能实现。
...
- 每条通知必须与产生它的业务写入放在同一数据库事务或同一原子 SQL 中，不能出现业务成功但通知写入失败的半完成状态。
...
该接口必须 `no-store`，按 `created_at DESC, id DESC` 排序，只读取当前会话用户的数据。
...
7. 在同一事务插入 `project_suggestion_received` 通知。
8. 提交后失效公开建议、项目详情和 Owner 建议页。
9. 返回结构化成功状态，不重定向。

项目行锁与管理员下架动作使用相同的项目行，保证“提交建议”和“下架项目”并发时得到确定结果：先下架则提交失败；先提交则建议和通知完整提交，随后项目下架。
...
6. 同一事务写入给提交者的状态通知。
7. 提交后刷新收到/提交的建议页、项目详情和公开建议 Cache Tag。

两个并发 Owner 请求会串行获取建议行锁。只有第一个合法转换成功，第二个读取新状态后返回安全的预期错误，不产生重复通知。

### 8.3 点赞通知

当前 `toggleLikeAction` 使用单条 CTE 原子切换。通知必须扩展在同一 SQL 中：

- 只有 `added` CTE 实际插入点赞时才插入 `project_liked` 通知。
- `removed` 分支不创建或删除通知。
- 若点赞用户等于项目 Owner，则跳过通知。
- 点赞结果、通知和点赞数读取保持在同一个数据库批次/事务边界内。

不能先切换点赞再进行独立通知写入，否则网络错误会造成点赞成功但通知丢失。

### 8.4 项目审核通知

在以下现有事务中，与项目状态和 `moderation_logs` 一起插入通知：
...
通知接收者从锁定的项目行读取，不能由客户端传入。拒绝原因同时进入项目、审核日志和通知受控载荷。
...
完成标准：数据库可以阻止自我建议、非法状态和并发重复处理，业务与通知写入可原子回滚。
...
- 新增通知创建 helper 和查询。
- 接入点赞、建议和项目审核事务。
- 实现 Header 未读数量、通知抽屉、全部已读和分页。
- 验证没有轮询或实时依赖。

完成标准：所有定义事件都只创建一次正确通知，打开通知后全部未读清零。
