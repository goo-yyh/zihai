...
Account creation requires a verified `qq.com` or `163.com` email OTP, GitHub, or Google identity. Direct password sign-up stays disabled. The email OTP identity allowlist is enforced before the client request and again in Better Auth's server-side delivery callback; matching is case-insensitive and exact, so subdomains and lookalike suffixes are rejected. After the first identity check, onboarding adds a username and password, confirms the avatar, and stores a private contact email. Existing OAuth-only users can add a password from security settings. Email OTP uses Resend through `src/server/auth-email.ts`; codes expire after five minutes, allow three attempts, are stored hashed, and sending is scheduled with Next.js `after()` so response timing does not reveal delivery behavior.
